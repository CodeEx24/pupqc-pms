...
function StudentScreen() {
  ...
  const {
    data: studentPassersList,
    isLoading,
    refetch: refetchStudentPassersList,
  } = useQuery(['studentPassersList'], () => fetchStudentPassersList());
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setValue('scores', '');
      setValue('examTypeVal', '');
      setValue('student_id', '');

      await refetchStudentPassersList();
  ...
  return (
    <AdminLayout title="Class Assign">
      <div className="bg-white p-10 rounded-xl">
        ...
        {submitting && <Processing text="Adding the students passer" />}
      </div>
    </AdminLayout>
  );
}
